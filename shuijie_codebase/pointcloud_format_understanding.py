...
# ============================================================================
# METHOD 1: Optimized NumPy (No dependencies, ~5-10x faster)
# ============================================================================

def fps(points, colors, num_samples=1024):
    """
    Fast FPS using optimized NumPy operations

    Speed: ~0.1-0.5s for 150k points -> 512 samples
    """
    N = points.shape[0]
    ...
    # Start with point closest to centroid (more deterministic than random)
    centroid = points.mean(axis=0)
    current_idx = np.argmin(np.sum((points - centroid) ** 2, axis=1))

    for i in range(num_samples):
        sampled_indices[i] = current_idx

        # Vectorized distance calculation
        dist = np.sum((points - points[current_idx]) ** 2, axis=1)

        # Update distances (in-place for speed)
        distances = np.minimum(distances, dist)

        # Find farthest point
        current_idx = np.argmax(distances)

    return points[sampled_indices], colors[sampled_indices], sampled_indices


@jit(nopython=True, parallel=True, fastmath=True)
def fps_numba_core(points, num_samples):
    """
    Numba-accelerated FPS core (points only)

    Speed: ~0.01-0.05s for 150k points -> 512 samples
    """
    N = points.shape[0]
    sampled_indices = np.zeros(num_samples, dtype=np.int32)
    distances = np.full(N, 1e10, dtype=np.float32)
...
def fps_numba(points, colors, num_samples=512):
    """
    Wrapper for numba FPS
    ...
    sampled_indices = fps_numba_core(points, num_samples)
